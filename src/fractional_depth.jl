...
import ..SampleAnalytics: SamplePayload, exact_quantile
...
Compute a sample quantile at fractional depth for a SamplePayload tree.

Uses `exact_quantile` at each ancestor level and interpolates the quantile
values. The result is marked as approximate (non-integer depth always implies
